...
import { contactApplicant } from "@/lib/actions/applications"
...
export default function ContactButton({
  ...
  const [contacted, setContacted] = useState(alreadyContacted)

  async function handle() {
    setLoading(true)
    try {
      await contactApplicant(applicationId)
      setContacted(true)
  ...
  return (
    <button
      onClick={handle}
      disabled={loading}
      className="inline-flex items-center gap-1.5 text-xs font-semibold text-white px-3 py-1.5 rounded-lg disabled:opacity-60 transition-colors"
    ...
    >
      ...
      {loading ? "Abrindo..." : "Entrar em contato"}
    </button>
  )
}
