...
function whatsappUrl(phone: string, shift: Shift) {
  const msg = encodeURIComponent(
    `Ola! Vi seu plantao no Shiftada e tenho interesse.\n\nPlantao: ${shift.title}\nData: ${formatDate(shift.date)}\nHorario: ${shift.start_time} - ${shift.end_time}\nCidade: ${shift.city}`
  )
  ...
  return (
    <div className="px-4 pt-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Plantoes</h1>
          <p className="text-sm text-gray-500 mt-0.5">{shifts.length} disponivel{shifts.length !== 1 ? "is" : ""}</p>
        </div>
      </div>
